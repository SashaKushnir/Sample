...
export const spaces = {
    ...
    getOrderedSpaces: (beg_datetime: string, end_datetime: string, banquet_id: number = 0, token: string) => {
        return myGetInstance.get<ApiGetBasicSpacesResponseType>(`/spaces?trashed=without&beg_datetime=${beg_datetime}&end_datetime=${end_datetime}&banquet_id[except]=${banquet_id}`, {
            headers: {
                'api-token': token
            }
        })
