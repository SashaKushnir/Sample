...
export const SpaceI: React.FC<SpaceIProps> = ({ spaceI, editMode }) => {
  const d = useDispatch();
  const beg_datetime = useSelector(
    (state: RootState) => state.banquet.beginning
  );
  ...
  return (
    <div
      className={`${
        !spaceI.disabled ? (spaceI.selected ? s.selected : "") : s.disabled
      } ${s.spaceI}`}
      ...
      <div>
        {spaceI.name === "Table" && (
          <div>
            ...
            <div className={s.price}>
              <b>Номер: {spaceI.number}</b>
            </div>
            <div className={s.price}>
              <b>Поверх: {spaceI.floor}</b>
            </div>
          </div>
        )}
        {spaceI.name !== "Table" && (
